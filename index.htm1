...
<script>
    ...
    const events = [
        ...
        { name: "암태도 소작쟁의", year: 1923, month: 0 },
        ...
        { name: "제1차 국공합작", year: 1924, month: 0 },
        ...
        { name: "함경선", year: 1928, month: 0 },
        { name: "원산 총파업", year: 1929, month: 0 },
        ...
        { name: "제 1차 미소 공동 위원회 개최", year: 1946, month: 3 },
        { name: "제 1차 미소 공동 위원회 결렬", year: 1946, month: 5 },
        { name: "이승만의 정읍 발언", year: 1946, month: 6 },
        { name: "좌우 합작 운동", year: 1946, month: 7 },
        { name: "좌우 합작 위원회 결성", year: 1946, month: 7 },
        { name: "트루먼 독트린 발표", year: 1947, month: 3 },
        { name: "제 2차 미소 공동 위원회 개최", year: 1947, month: 5 },
        { name: "한반도 문제가 유엔으로 이관", year: 1947, month: 9 },
        { name: "제 2차 미소 공동 위원회 결렬", year: 1947, month: 10 },
        { name: "한반도 총선거 실시 결정", year: 1947, month: 11 },
        { name: "남한만의 단독 선거 실시 결정", year: 1948, month: 2 },
        { name: "김구의 남북 협상", year: 1948, month: 4 },
    ...
    function renderTimeline(searchTerm = '') {
        ...
        let eventsToRender;
        if (normalizedSearchTerm === '박찬일') {
            eventsToRender = events.filter(event => event.name.includes('미소'));
        } else if (normalizedSearchTerm === '곽승헌') {
            eventsToRender = events.filter(event => event.name.includes('태형'));
        } else if (normalizedSearchTerm === '노무현') {
            eventsToRender = events.filter(event => event.year === 1946);
        } else {
            eventsToRender = normalizedSearchTerm ? events.filter(event => 
                event.name.toLowerCase().includes(normalizedSearchTerm) || 
                String(event.year).includes(normalizedSearchTerm)
            ) : events;
        }
    ...
    function checkAnswer() {
        ...
        // '다음 문제' 버튼 생성
        const quizActions = document.getElementById('quiz-actions');
        quizActions.innerHTML = ''; // 기존 버튼 제거
